...
def create_crypto_invoice(uid, amount_rub):
    ...
    payload = {"asset": "TON", "amount": str(amount_rub), "description": f"Clash Farm. ID: {uid}", "payload": uid, "allow_comments": False}
    try:
...
def check_crypto_payment(invoice_id):
    ...
    headers = {"Crypto-Pay-API-Token": CRYPTO_TOKEN}
    try:
        r = requests.get(url, params={"invoice_ids": str(invoice_id)}, headers=headers, timeout=10).json()
        ...
        try:
            for inv_id, inv_data in list(data["invoices"].items()):
                if check_crypto_payment(int(inv_id)):
                    ...
                    save()
                    try:
                        bot.send_message(uid, f"✅ +{diamonds:.0f}💎 | Баланс: {data['users'][uid]['bal']:.1f}💎")
                    except:
                        pass
        except:
            pass
...
def start(message):
    ...
    if ref and ref != uid and ref in data["users"]:
        if not data["users"][uid]["ref"]:
            ...
            save()
    bot.send_message(message.chat.id, f"👋 Clash Farm!\n💰 +{inc:.1f}💎", reply_markup=menu())
...
def depo(message):
    uid = str(message.from_user.id)
    kb = telebot.types.InlineKeyboardMarkup()
    kb.add(telebot.types.InlineKeyboardButton("💳 CryptoBot (авто)", callback_data="crypto_pay"))
    kb.add(telebot.types.InlineKeyboardButton("💼 TON вручную", callback_data="manual_depo"))
    bot.send_message(message.chat.id, "Выберите способ:", reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data == "crypto_pay")
def crypto_pay(call):
    msg = bot.send_message(call.message.chat.id, "Сумма в рублях (мин 50₽):")
...
def create_payment(message):
    ...
    try:
        ...
        if amount < 50:
            return bot.send_message(message.chat.id, "❌ Мин 50₽")
    except:
        return bot.send_message(message.chat.id, "❌")
    url, _ = create_crypto_invoice(uid, amount)
    if url:
        kb = telebot.types.InlineKeyboardMarkup()
        kb.add(telebot.types.InlineKeyboardButton("💳 Оплатить", url=url))
        bot.send_message(message.chat.id, f"Счёт на {amount}₽ создан!\nОплатите и 💎 зачислятся автоматически.", reply_markup=kb)
    else:
...
def manual_depo(call):
    uid = str(call.from_user.id)
    bot.send_message(call.message.chat.id, f"💼 TON\n{TON_WALLET}\n⚠️ Коммент: ID {uid}")
...
def ab2(message):
    try:
        ...
        reg(p[0]); data["users"][str(p[0])]["bal"] += float(p[1]); save()
        bot.send_message(message.chat.id, "✅")
    except:
        bot.send_message(message.chat.id, "❌")
...
def rb2(message):
    try:
        p = message.text.split()
        uid = str(p[0]); amt = float(p[1])
        data["users"][uid]["bal"] = max(0, data["users"][uid]["bal"] - amt); save()
        bot.send_message(message.chat.id, "✅")
    except:
        bot.send_message(message.chat.id, "❌")
...
def br2(message):
    ...
    for uid in data["users"]:
        try: bot.send_message(uid, message.text); c += 1
        except: pass
