...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hero-img { width: 55px; height: 55px; border-radius: 12px; margin-right: 10px; background: #1a1a3e; overflow: hidden; flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
        ...
        .btn-del { background: var(--red); color: #fff; padding: 4px 8px; border-radius: 6px; border: none; cursor: pointer; font-size: 11px; margin-left: 5px; }
        ...
        .file-label { background: var(--blue); color: #fff; padding: 10px; border-radius: 8px; text-align: center; cursor: pointer; display: block; margin: 5px 0; }
        .preview-img { width: 80px; height: 80px; border-radius: 10px; object-fit: cover; margin: 5px 0; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <script>
        ...
        let userId = TG?.initDataUnsafe?.user?.id ? String(TG.initDataUnsafe.user.id) : (localStorage.getItem('cf_userId') || 'demo');
        if (!localStorage.getItem('cf_userId')) localStorage.setItem('cf_userId', userId);
        ...
        function renderHeroes() {
            ...
            heroes.forEach((hero, idx) => {
                ...
                let imgHtml = hero.img ? `<img src="${hero.img}" alt="">` : '🖼';
                h += `<div class="hero-item ${owned ? 'owned' : ''}">
                    <div class="hero-img">${imgHtml}</div>
                    <div class="hero-info">
                        <div class="hero-name">${hero.name} ${isAdmin ? `<button class="btn-del" onclick="delHero(${idx})">🗑</button>` : ''}</div>
        ...
        function delHero(idx) {
            if (!confirm('Удалить героя: ' + heroes[idx].name + '?')) return;
            heroes.splice(idx, 1);
            saveHeroes();
            refreshAll();
        }
        ...
        function handleHeroImage(e) {
            ...
            reader.onload = function(ev) {
                heroImageData = ev.target.result;
                document.getElementById('previewImg').innerHTML = `<img src="${heroImageData}" class="preview-img">`;
            };
        ...
        function addHero() {
            ...
            heroes.push({id: 'h' + Date.now(), name, img: heroImageData, price, income, desc});
            saveHeroes();
            heroImageData = '';
            document.getElementById('previewImg').innerHTML = '';
            document.getElementById('hName').value = '';
            document.getElementById('hPrice').value = '';
            document.getElementById('hIncome').value = '';
        ...
        function renderRefs() {
            ...
            return `<h3>👥 Реферальная система</h3>
                <div class="card">
                    ...
                    <button class="btn btn-gold" onclick="copyText('${link}')">📋 Скопировать</button>
                    <p class="info">👤 Рефералов: <b>${data.refs.length}</b></p>
                    <p class="info">💎 +1💎 за 5 заданий | 💸 +5% от пополнений</p>
                </div>`;
        }
        ...
        function renderDepo() {
            return `<h3>💎 Пополнение через TON</h3>
                <div class="card">
                    ...
                    <p>📌 <b>Кошелёк:</b></p>
                    <div class="link-box" onclick="copyText('${tonWallet}')">${tonWallet}</div>
                    <button class="btn btn-gold" onclick="copyText('${tonWallet}')">📋 Скопировать адрес</button>
                    <p class="warning">⚠️ В комментарии укажите ID: <b>${userId}</b></p>
        ...
        function renderWithdraw() {
            return `<h3>💸 Вывод</h3>
                <div class="card">
                    <p>📌 100💎 = 1 TON | 5% | Мин: 50💎</p>
                    <p>💰 Баланс: <b>${data.bal.toFixed(2)}💎</b></p>
                    <input class="input" id="wdAmt" type="number" placeholder="Сумма">
        ...
        function withdraw() {
            ...
            if (!amt || amt < 50 || amt > data.bal) return alert('❌');
            if (!wal) return alert('❌ Кошелёк!');
            let ton = (amt * 0.95) / 100;
            data.bal -= amt;
            save();
            updateUI();
            refreshAll();
            alert(`✅ ${ton.toFixed(2)} TON`);
        }
        ...
        function renderAdmin() {
            ...
            return `<h3>🔧 Админ-панель</h3>
                <div class="card">
                    ...
                    <label class="file-label" onclick="document.getElementById('heroImg').click()">📷 Выбрать картинку</label>
                    <input type="file" id="heroImg" class="file-input" accept="image/*" onchange="handleHeroImage(event)">
                    <div id="previewImg"></div>
                    <input class="input" id="hPrice" type="number" placeholder="Цена 💎">
                    <input class="input" id="hIncome" type="number" placeholder="Доход/час" step="0.01">
                    <input class="input" id="hDesc" placeholder="Описание">
                    <button class="btn btn-buy" onclick="addHero()">✅ Добавить</button>
                </div>
                <div class="card">
                    <h4>📋 Задание</h4>
                    ...
                    <button class="btn btn-buy" onclick="addTask()">✅ Добавить</button>
                </div>
                <div class="card">
                    <h4>🎫 Промокод</h4>
                    <input class="input" id="pCode" placeholder="Код">
                    <input class="input" id="pReward" type="number" placeholder="Награда">
                    <input class="input" id="pUses" type="number" placeholder="Активаций">
        ...
        function addTask() {
            ...
            if (!name || !link || !reward) return alert('❌');
        ...
        function changeBal(sign) {
            ...
            if (!uid || !amt) return alert('❌');
            let u = JSON.parse(localStorage.getItem('cf_' + uid) || '{"bal":0,"heroes":["default"],"htime":' + Date.now() + ',"btime":0,"tasks":[],"refs":[],"promosUsed":[]}');
            u.bal += amt * sign;
            if (u.bal < 0) u.bal = 0;
            localStorage.setItem('cf_' + uid, JSON.stringify(u));
            if (uid === userId) { data = u; save(); updateUI(); }
            alert(`✅ ${sign > 0 ? '+' : '-'}${amt}💎 → ${uid}`);
            refreshAll();
        }
